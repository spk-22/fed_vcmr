...
def calculate_iou(pred, target):
    """pred, target: [start, end] normalized"""
    p_s, p_e = pred
    t_s, t_e = target
    inter_s = max(p_s, t_s)
    inter_e = min(p_e, t_e)
...
def main():
    ...
    segments = conn.execute('SELECT video_id, sentence, start_time, end_time, duration, feature_path FROM anet_val_segments').fetchall()
    conn.close()
    
    if not segments:
        print("Error: No validation segments found in DB.")
        return

    # 3. Build Video-level FAISS Index
    print('Building FAISS index for validation subset...')
    unique_vids = sorted(list(set([s[0] for s in segments])))
    if args.subset:
        unique_vids = unique_vids[:args.subset]
        print(f"  Subsetting to first {len(unique_vids)} videos")
    
    vid_feats = []
    indexed_vids = [] # To keep track of what's actually in FAISS
    w = np.array([0.5, 0.75, 1.0, 1.25, 1.25, 1.0, 0.75, 0.5])
    
    for vid in tqdm(unique_vids, desc="Indexing Videos"):
        feat_path = f"cache/anet_val_frames/{vid}.npy"
        if not os.path.exists(feat_path):
            continue 
        frames = np.load(feat_path).astype(np.float32) # (8, 512)
        # Weighted avg
    ...
    # 4. Evaluation Loop
    print(f'Starting VCMR evaluation on {len(segments)} samples...')
    hits_05 = 0
    hits_07 = 0
    retrieval_hits = 0
    
    # Cache for frame features to avoid redundant loads
    frame_cache = {vid: np.load(f"cache/anet_val_frames/{vid}.npy").astype(np.float32) for vid in indexed_vids}

    for vid, sentence, s_time, e_time, duration, _ in tqdm(segments, desc="Evaluating"):
        if vid not in frame_cache:
            continue # Skip segments for which we couldn't download the video
    ...
    r1_iou05 = hits_05 / len(segments)
    r1_iou07 = hits_07 / len(segments)
    vid_r1 = retrieval_hits / len(segments)
    
    print(f'\n{"="*40}')
    print(f'ACTIVITYNET VCMR RESULTS (Subset=500)')
    print(f'{"="*40}')
    print(f'Video R@1 (Retrieval): {vid_r1*100:.2f}%')
    print(f'R@1 IoU@0.5: {r1_iou05*100:.2f}%')
    print(f'R@1 IoU@0.7: {r1_iou07*100:.2f}%')
    print(f'MA-VR Baseline (IoU@0.7): 10.04%')
    if r1_iou07 > 0:
        print(f'Improvement: { (r1_iou07/0.1004 - 1)*100 :.1f}%')
    print(f'{"="*40}\n')
